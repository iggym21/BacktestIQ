  ...
  return (
    <Layout>
      <div className="max-w-5xl mx-auto">
        <h2 className="text-2xl font-bold text-ink mb-1">Walk-Forward Validation</h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            <label htmlFor="wf-ticker" className="block text-xs text-ink-muted mb-1">Ticker</label>
          ...
          <div>
            <label htmlFor="wf-start-date" className="block text-xs text-ink-muted mb-1">Start Date</label>
          ...
          <div>
            <label htmlFor="wf-end-date" className="block text-xs text-ink-muted mb-1">End Date</label>
            <input id="wf-end-date" type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)}
              className="surface-input w-full px-3 py-2 text-sm" />
          </div>
          <div>
            <label htmlFor="wf-capital" className="block text-xs text-ink-muted mb-1">Capital ($)</label>
          ...
          <div>
            <label htmlFor="wf-benchmark" className="block text-xs text-ink-muted mb-1">Benchmark</label>
        ...
        <div className="card p-5 mb-6">
          <h3 className="text-ink font-semibold mb-4">Fold Configuration</h3>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label htmlFor="wf-folds" className="block text-xs text-ink-muted mb-1">Number of folds</label>
              <input id="wf-folds" type="number" min={2} max={12} value={folds}
                onChange={(e) => setFolds(Number(e.target.value))}
                className="surface-input w-full px-3 py-2 text-sm font-mono" />
            </div>
            <div>
              <label htmlFor="wf-metric" className="block text-xs text-ink-muted mb-1">Metric to chart</label>
        ...
        {result && (
          <div className="space-y-6">
            {worst !== null && (
              <div className="rounded-xl p-4 text-sm border" style={
                lostMoney
                  ? { background: "var(--negative-soft)", borderColor: "var(--negative)", color: "var(--negative)" }
                  : { background: "var(--positive-soft)", borderColor: "var(--positive)", color: "var(--positive)" }
              }>
                {metric.label} ranged from <span className="font-semibold font-mono">{metric.pct ? `${Math.min(...values).toFixed(2)}%` : Math.min(...values).toFixed(3)}</span> to{" "}
                <span className="font-semibold font-mono">{metric.pct ? `${Math.max(...values).toFixed(2)}%` : Math.max(...values).toFixed(3)}</span> across folds
            ...
            {chartData.length > 0 && (
              <div className="card p-5">
                <h3 className="text-ink font-semibold mb-4">{metric.label} by Fold</h3>
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart data={chartData} margin={{ left: 8, bottom: 8 }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={ct.grid} />
                    <XAxis dataKey="fold" tick={{ fill: ct.tick, fontSize: 11 }} tickLine={false}
                      label={{ value: "Fold", position: "insideBottom", offset: -6, fill: ct.axisLabel, fontSize: 11 }} />
                    ...
                    <Bar dataKey="metric" radius={[4, 4, 0, 0]}>
                      {chartData.map((d, i) => (
                        <Cell key={i} fill={d.metric >= 0 ? ct.brand : ct.negative} />
                      ))}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
            ...
            <div className="card p-5 overflow-x-auto">
              <h3 className="text-ink font-semibold mb-4">All Folds</h3>
              <table className="w-full text-sm font-mono">
                <thead>
                  <tr className="text-ink-muted text-left border-b" style={{ borderColor: "var(--line)" }}>
                    <th className="py-2 pr-4 font-sans font-medium">Fold</th>
                    <th className="py-2 pr-4 font-sans font-medium">Period</th>
                ...
                <tbody>
                  {result.map((f) => (
                    <tr key={f.fold} className="border-b" style={{ borderColor: "var(--line)" }}>
                      <td className="py-2 pr-4 text-ink font-medium">#{f.fold}</td>
                      <td className="py-2 pr-4 text-ink-muted text-xs">{f.start_date} – {f.end_date}</td>
                      {f.error ? (
                        <td colSpan={4} className="py-2 pr-4 text-negative font-sans">{f.error}</td>
                      ) : (
                        <>
                          <td className={`py-2 pr-4 ${f.metrics!.total_return >= 0 ? "text-positive" : "text-negative"}`}>
                            {(f.metrics!.total_return * 100).toFixed(2)}%
                          </td>
                          <td className="py-2 pr-4 text-ink-muted">{f.metrics!.sharpe.toFixed(3)}</td>
                          <td className="py-2 pr-4 text-negative">{(f.metrics!.max_drawdown * 100).toFixed(2)}%</td>
                          <td className="py-2 pr-4 text-ink-muted">{f.metrics!.num_trades}</td>
                        </>
                      )}
                    </tr>
                  ))}
